...
{: .motivation }
A very important notion in probability theory is independence. Intuitively, when two events are independent, the outcome
of either event does not inform us about the outcome of the other event. This will allow us to 1) simplify many of our 
calculations when doing machine learning, and 2) help us to parametrize out joint distributions in a more efficient way. 
In this section, we will look at what it means for two variables to be independent, and then expand that understanding
of why studying when two variables are independent conditional on some extra information. 



## Basic independence

Going back to our initial example with a customer rating our movie on a scale of $$1$$ to $$5$$, denoted as $$R$$, and them
...
However, we could imagine a situation where someone gives a higher rating for a movie because they are happy, i.e. if we
...
Similarly, if $$S_1$$ is not on time, I increase my belief that $$S_2$$
...
In the case above, we had a situation where $$X$$ and $$Y$$ were dependent, but $$X$$ and $$Y$$ were independent when informed about some $$Z$$. 
I.e., even if there is an 'information flow' between $$X$$ and $$Y$$, there can be no 'information flow' from $$X$$ to $$Y$$ if 
we know that $$Z$$. The other case is also possible, the situation where $$X$$ and $$Y$$ are independent variables, 
but they become dependent given some variable $$Z$$. To illustrate this, let us consider a new - quite inspired on our breakfast - scenario.

Suppose that $$S$$ is the random variable that describes whether or not the first strawberry I eat on a day is sweet.
