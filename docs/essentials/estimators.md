...
distribution models a weighted coin flip: it has one parameters $$\theta$$, which denotes the probability of $$1$$, and assigns
...
Sometimes - especially since many distribution have exponential in them - it is easier to optimize an equivalent objective, 
...
and now wish to find the $$\theta$$. Since the outcomes $$x$$ are those that actually occured, it makes sense to find the
...
These is one last subtly we need to cover. The above gives us the likelihood for $$\theta$$ given a single datapoint. 
Commonly when doing MLE, the following assumption is made: **all datapoints are identically and independently distributed**.
...
in which we collected our data is non important, i.e. if we shuffle the data around it should not matter. This makes sense for
...
each such observation is eiter $$1$$ if the flip was positive, or $$0$$ is the flip was negative. Intuively, we might guess
that $$\theta$$ should be the number of positive flips relative to the total number of observations, but let us see the MLE 
tells us. 
...
Taking the derivative is quite straight forward, since
...
Now, we are interested in setting this derivative equal to zero, to see where the optimmum lies, which gives us 
...
which you can calculate simply by summing over the observations - divided by the total number of observations. 