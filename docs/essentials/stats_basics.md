...
---
{: .content }
In this section you will what probabilities, random variables, (joint) distributions, and queries are.
...
We call the way these probabilities are divides over the outcomes a **distribution** of the random variable.
...
a continuous random variables. Technically, we say that the probability of $$0$$, but the event is not impossible to occur, but do
not worry too much about this.

But what if we actually want to compute values for our random variable? Suppose the outcomes of our event $$X$$ 
...
will lie between those to points. To obey the same rules as mentioned above, we just assume that 1) the total area underneath this 
...
{: .summary }
In this section, you have seen how to formalize uncertain events and you have covered the most basic queries one can ask based on a joint distribution we have of our events.