...
In the last section we saw that we can take a joint probability and write is as 
...
$$\mathbb{P}(X_1, \cdots, X_n) = \prod_{k=1}^n \mathbb{P}(X_k \mid X_1, \cdots, X_{k-1}).$$
...
$$\mathbb{P}(Y) \cdot \mathbb{P}(X \mid Y) = \mathbb{P}(X) \cdot \mathbb{P}(Y \mid X) \implies \mathbb{P}(X \mid Y) = \frac{\mathbb{P}(Y \mid X) \mathbb{P}(X)}{\mathbb{P}(Y)}.$$ 
